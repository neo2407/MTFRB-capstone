...
$notifications_sql = "SELECT id, message FROM notifications WHERE seen = 0 AND type = 'update' ORDER BY id DESC";
$notifications_result = mysqli_query($conn, $notifications_sql);
$notifications = mysqli_fetch_all($notifications_result, MYSQLI_ASSOC);

// Combine both counts
$total_notifications = count($new_applications) + count($new_complaints) + count($notifications)  ;
