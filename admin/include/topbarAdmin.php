...
<li class="nav-item dropdown no-arrow">
    <a class="nav-link dropdown-toggle" href="#" id="userDropdownToggle" role="button"
        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <img class="img-profile rounded-circle"
             ...
             height="28" alt="Admin Profile" loading="lazy" />
    </a>
    ...
    <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" 
        ...
        <a class="dropdown-item" href="../dashboard/admin_logout.php" id="logoutLink" data-toggle="modal" data-target="#logoutModal">
                ...
                <form action="editInfo_account.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-8">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="username" name="username" value="<?php echo isset($_SESSION['username']) ? $_SESSION['username']: 'Usernames';?>">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="account_type" name="account_type" value="<?php echo isset($_SESSION['account_type']) ? $_SESSION['account_type'] : 'Account Type'; ?>">
...
//notification dropdown and userdropdown
$(document).ready(function () {
    $('.dropdown-toggle').dropdown();
});

$(document).ready(function () {
    $('.userDropdownToggle').dropdown();
});
...
// Detect when the user clicks a link and update the flag
document.querySelectorAll('a').forEach(link => {
    link.addEventListener('click', function (e) {
        const linkHref = e.target.getAttribute('href');

        // Check if the link leads outside the SuperAdmin section
        if (linkHref && !linkHref.startsWith("/MTFRB/superAdmin/")) {
            isNavigatingAway = true; // Set the flag only for external links
        }
    });
});

// Handle visibility change (browser tab hidden)
document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'hidden' && !isNavigatingAway) {
        console.log("Page hidden, but not navigating away.");
    }
});

// Handle page unload (browser/tab close detection)
window.addEventListener('beforeunload', function (e) {
    if (!isNavigatingAway && !sessionStorage.getItem('pageReloaded')) {
        console.log("Browser/tab close detected, sending logout request.");
        sendLogout(); // Send logout only on tab close, not on reload
    } else {
        console.log("Page reloaded or navigating within the section, no logout recorded.");
    }
});

// Track page reloads using sessionStorage
window.addEventListener('load', function () {
    console.log("Page loaded, resetting navigation flag.");

    // Set a flag in sessionStorage to identify reloads
    sessionStorage.setItem('pageReloaded', 'true');

    // Clear the reload flag after a short delay to ensure it's only for reload detection
    setTimeout(() => sessionStorage.removeItem('pageReloaded'), 100);
