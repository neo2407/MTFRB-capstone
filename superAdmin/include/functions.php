...
<script>
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        let notifications = data.notifications;
          let dropdownMenu = $('#alertsDropdown').next('.dropdown-menu');
        dropdownMenu.empty();
    
        
        // Display new complaints
        complaints.forEach(complaints => {
          dropdownMenu.append('<li><a class="dropdown-item" href="#" onclick="markComplaintsAsSeen(' + complaints.id + '); return false;">New complaint from ' + complaints.first_name + ' ' + complaints.last_name + '</a></li>');
        });

        // Display other notifications
        notifications.forEach(notification => {
          dropdownMenu.append('<li><a class="dropdown-item" href="#" onclick="markNotificationAsSeen(' + notification.id + '); return false;">' + notification.message + '</a></li>');
        });
      },
  ...
  // Function to mark Complaints as seen
    ...
    .then(data => {
        ...
        if (data.includes("Record updated successfully")) {
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: 'File status updated successfully!',
            });
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to update file status.',
            });
        }
    })
    .catch(error => {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'An error occurred while updating the file status.',
        });
    });
}
...
// send SMS infobip function
function sendSMSinfobip() { 
    // Show loading spinner
    document.getElementById('loadingSpinner').style.display = 'block';
