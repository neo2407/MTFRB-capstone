<?php
include "../../include/db_conn.php";


// Automatically update interviewStatus based on the interview date
$currentDate = date('Y-m-d H:i:s');
$updateStatusQuery = "
    UPDATE applicants
    SET interviewStatus = CASE
        WHEN interview_sched IS NULL OR interview_sched = '' THEN 'Pending'
        WHEN interview_sched < '$currentDate' THEN 'Missed'
        ELSE 'Pending'
    END
    WHERE interviewStatus != 'Done'";
mysqli_query($conn, $updateStatusQuery);

// Fetch filter values from the request
$firstName = isset($_GET['first_name']) ? $_GET['first_name'] : '';
$lastName = isset($_GET['last_name']) ? $_GET['last_name'] : '';
$tricColor = isset($_GET['tricColor']) ? $_GET['tricColor'] : '';
$interviewStatus = isset($_GET['interviewStatus']) ? $_GET['interviewStatus'] : '';
$paymentStatus = isset($_GET['paymentStatus']) ? $_GET['paymentStatus'] : '';
$type = isset($_GET['tricType']) ? $_GET['tricType'] : '';


// Pagination parameters
$page = isset($_GET['page']) ? intval($_GET['page']) : 1;
$pageSize = isset($_GET['pageSize']) ? intval($_GET['pageSize']) : 10;
$offset = ($page - 1) * $pageSize;

// Ensure pagination parameters are valid
$page = max(1, $page); // Ensure page is at least 1
$pageSize = max(1, $pageSize); // Ensure pageSize is at least 1
$offset = max(0, $offset); // Ensure offset is not negative


$sql = "SELECT * FROM applicants WHERE applicantStatus='Verified'";
$countSql = "SELECT COUNT(*) as total FROM applicants WHERE (applicantStatus='Verified')";
// Add conditions based on filters
if (!empty($firstName)) {
    $sql .= " AND first_name LIKE '%" . mysqli_real_escape_string($conn, $firstName) . "%'";
    $countSql .= " AND first_name LIKE '%" . mysqli_real_escape_string($conn, $firstName) . "%'";
}

if (!empty($lastName)) {
    $sql .= " AND last_name LIKE '%" . mysqli_real_escape_string($conn, $lastName) . "%'";
    $countSql .= " AND last_name LIKE '%" . mysqli_real_escape_string($conn, $lastName) . "%'";
}

if (!empty($interviewStatus)) {
    $sql .= " AND interviewStatus = '" . mysqli_real_escape_string($conn, $interviewStatus) . "'";
    $countSql .= " AND interviewStatus = '" . mysqli_real_escape_string($conn, $interviewStatus) . "'";
}

if (!empty($paymentStatus)) {
    $sql .= " AND paymentStatus = '" . mysqli_real_escape_string($conn, $paymentStatus) . "'";
    $countSql .= " AND paymentStatus = '" . mysqli_real_escape_string($conn, $paymentStatus) . "'";
}

if (!empty($type)) {
    $sql .= " AND tricType = '" . mysqli_real_escape_string($conn, $type) . "'";
}

if (!empty($tricColor)) {
    $sql .= " AND tricColor LIKE '%" . mysqli_real_escape_string($conn, $tricColor) . "'";
    $countSql .= " AND tricColor LIKE '%" . mysqli_real_escape_string($conn, $tricColor) . "%'";
}

// Add ORDER BY clause after conditions
$sql .= " ORDER BY applicationDate DESC";

// Count total records for pagination based on the filtered query
$countResult = mysqli_query($conn, $countSql);
if (!$countResult) {
    die('Error counting records: ' . mysqli_error($conn));
}
$totalRow = mysqli_fetch_assoc($countResult);
$totalRecords = $totalRow['total'];

// Append limit and offset for pagination to the main query
$sql .= " LIMIT $offset, $pageSize";

// Execute the main query
$result = mysqli_query($conn, $sql);
if (!$result) {
    die('Error executing query: ' . mysqli_error($conn));
}

// Fetch results as an associative array
$data = mysqli_fetch_all($result, MYSQLI_ASSOC);

// Calculate total pages
$totalPages = ceil($totalRecords / $pageSize);

// Return the results as JSON
header('Content-Type: application/json');
echo json_encode([
    'rows' => $data,
    'totalPages' => $totalPages,
    'currentPage' => $page // Including the current page for reference
]);

mysqli_close($conn);
?>